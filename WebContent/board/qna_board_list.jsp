...
<section id="pagelist">
	<%if(nowPage <= 1) {%> [이전]%nbsp; 
	<%}else{ %> <a href="boardList.bo?page=<%=nowPage-1%>">[이전]</a>%nbsp; <%} %>
	
	<!-- 처음페이지~마지막페이지 번호까지 출력(예)nowPage=2일때 startPage=1,endPage=10   noePage=15일때 startPage=11, endPage=20 -->
	<%for(int i=startpage;i<=endPage;i++){ 
	 	if(i==nowPage){%> [<%=i%>]&nbsp;
	 	<%}//if
	 		else{%>
	 			<a href="boardList.bo?=<%=i%>">[<%=i%>]</a>&nbsp;
	 	<%} %>
	<%}//for %>
	
	<%if(nowPage>=maxPage){ %> [다음]
	<%}else{ %>
		<a href="boardList.bo?page=<%=nowPage%>">[다음]</a>
	<%} %>
</section>
<% } else{%>
